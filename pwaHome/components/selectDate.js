...
import SelectDateOverlay from './selectDateOverlay';
  ...
  render() {
	...
	const options = { month: "short", day: "numeric" };
	    ...
	    return (
			...
			<p className="font14 latoBlack appendBottom4 makeFlex flexWrap"><span> {`${from.toLocaleDateString('en-GB', options)}`}{''}</span> - <span>{`${to.toLocaleDateString('en-GB', options)}`}{''}</span></p>
			</div>			
